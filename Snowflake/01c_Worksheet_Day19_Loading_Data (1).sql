...
SELECT
    ...
    CREATE
    OR REPLACE FILE FORMAT CSV_STD TYPE = CSV FIELD_DELIMITER = ',' SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"' NULL_IF = ('', 'NULL', 'null') EMPTY_FIELD_AS_NULL = TRUE;
SHOW FILE FORMATS;
    -- ---------------------------------------------------------------------
    ...
    CREATE
    OR REPLACE STAGE MY_STAGE FILE_FORMAT = CSV_STD COMMENT = 'Landing zone - Day 19';
LIST @MY_STAGE;
    -- empty. Nothing has landed yet.
    -- ---------------------------------------------------------------------
...
FROM
    (
        ...
        TYPE = CSV COMPRESSION = NONE FIELD_OPTIONALLY_ENCLOSED_BY = '"'
...
SELECT
    ...
    $5 AS orderdate
FROM
    @MY_STAGE/orders/orders_1.csv (FILE_FORMAT => CSV_STD)
LIMIT
    5;
...
FROM
    @MY_STAGE/orders/ FILE_FORMAT = (FORMAT_NAME = CSV_STD) ON_ERROR = 'ABORT_STATEMENT';
...
FROM
    ...
    COPY INTO ORDERS_RAW
FROM
    @MY_STAGE/orders/ FILE_FORMAT = (FORMAT_NAME = CSV_STD) FORCE = TRUE;
SELECT
    COUNT(*) AS rows_in_table
FROM
    ORDERS_RAW;
...
FROM
    TABLE(
        MY_DB.INFORMATION_SCHEMA.COPY_HISTORY(
...
FROM
    (
        SELECT
            TO_VARCHAR(O_ORDERKEY) AS c1,
            TO_VARCHAR(O_CUSTKEY) AS c2,
            O_ORDERSTATUS AS c3,
            CASE
                WHEN MOD(O_ORDERKEY, 7) = 0 THEN 'not_a_price'
    ...
    COPY INTO ORDERS_RAW
FROM
    @MY_STAGE/bad/ FILE_FORMAT = (FORMAT_NAME = CSV_STD) VALIDATION_MODE = 'RETURN_ERRORS';
    ...
    COPY INTO ORDERS_RAW
FROM
    @MY_STAGE/bad/ FILE_FORMAT = (FORMAT_NAME = CSV_STD) ON_ERROR = 'CONTINUE';
...
FROM
    ...
    -- YOUR CODE HERE
    -- 1. Unload 2000 rows from LINEITEM into the stage
    COPY INTO @MY_STAGE/lineitem/lineitem_1.csv
FROM
    (
        SELECT
            L_ORDERKEY,
    ...
    -- 2. Query the file before loading it
SELECT
    $1 AS orderkey,
    $2 AS partkey,
    $3 AS quantity,
    ...
    $5 AS shipdate
FROM
    @MY_STAGE/lineitem/lineitem_1.csv (FILE_FORMAT => CSV_STD)
LIMIT
    5;
    -- 3. Create LINEITEM_RAW table
    CREATE
    OR REPLACE TABLE LINEITEM_RAW (
        l_orderkey NUMBER,
        l_partkey NUMBER,
        l_quantity NUMBER,
    ...
    -- 4. Load the file into LINEITEM_RAW
    COPY INTO LINEITEM_RAW
FROM
    @MY_STAGE/lineitem/ FILE_FORMAT = (FORMAT_NAME = CSV_STD) ON_ERROR = 'ABORT_STATEMENT';
    -- Check the number of rows
SELECT
    COUNT(*) AS rows_in_table
FROM
    LINEITEM_RAW;
    -- 5. Run the SAME COPY again
    COPY INTO LINEITEM_RAW
FROM
    @MY_STAGE/lineitem/ FILE_FORMAT = (FORMAT_NAME = CSV_STD) ON_ERROR = 'ABORT_STATEMENT';
...
FROM
    TABLE(
        MY_DB.INFORMATION_SCHEMA.COPY_HISTORY(
            TABLE_NAME => 'MY_DB.RAW.LINEITEM_RAW',
            START_TIME => DATEADD(hour, -2, CURRENT_TIMESTAMP())
        )
    )
ORDER BY
